...
func (b *Bash) CreatePrompt() error {
	...
	promptContent := []string{
		...
		"",
		"# Highlight the host if this machine is accessed via SSH",
		"host=\"${orange}\\h\"",
		"if [[ -n \"${SSH_TTY}\" ]]; then",
		"    host=\"${yellow}remote:${red}\\h\"",
		"fi",
		"",
		"# Set the default interaction prompt",
		"PS1=\"${bold}\"												# bold",
		"PS1+=\"${white}[${yellow}\\A${white}]\"						# [time]",
		"PS1+=\"[${userColor}\\u${white}@${host}${white}]\"				# [user@host]",
		"PS1+=\"[${steel_blue}\\w${white}]\"							# [pwd]",
		"PS1+=\"\\n$ ${reset}\"											# new line, dollar sign and reset",
		"export PS1",
		"",
